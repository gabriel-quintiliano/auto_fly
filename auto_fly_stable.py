...
def _create_custom_dict(file, separator) -> dict:
    ...
    while (line := file.readline()) != b'':
        ...
        line = line.decode() #.split(separator)
...
def register_excecution_info():
    write_value_for_key(execution_info_file, ('reference_date', 'last_execution', 'by_user'), (ref_date, hoje.strftime('%d/%m/%Y %H:%M'), overall_options['user']), separator=':')
...
# Infelismente essa variável tem que estar aqui embaixo porque lá em cime fala que a função
# get_options ainda não está definida
